...
import { processGalleryImage, generateThumbnail } from "@/lib/image";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [processed, thumbnail] = await Promise.all([
      processGalleryImage(buffer),
      generateThumbnail(buffer),
    ]);

    // Save files to disk
    const [url, thumbnailUrl] = await Promise.all([
      saveFile(processed, user.code, "gallery"),
      saveFile(thumbnail, user.code, "gallery-thumb"),
    ]);
